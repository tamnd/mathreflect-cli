...
describes what users get. Pick a format with `-o`, or let mathreflect choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
mathreflect <command> -o table   # aligned columns for reading
mathreflect <command> -o jsonl   # one JSON object per line, for piping
mathreflect <command> -o json    # a single JSON array
mathreflect <command> -o csv     # spreadsheet friendly
mathreflect <command> -o tsv     # tab-separated
mathreflect <command> -o url     # just the URL column
mathreflect <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
mathreflect <command> --fields id,title,url
```
...
```bash
mathreflect <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
mathreflect <command>            # a table, because this is a terminal
mathreflect <command> | wc -l    # JSONL, because this is a pipe
```
